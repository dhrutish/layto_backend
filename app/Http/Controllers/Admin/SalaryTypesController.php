...
class SalaryTypesController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sql = SalaryTypes::orderBy($sort, $order);
            ...
            foreach ($res as $key => $row) {
                $action = '<i class="fa-regular fa-ellipsis cursor-pointer" data-bs-toggle="dropdown" aria-expanded="false"></i><ul class="dropdown-menu"><li><a class="dropdown-item edit-details" data-bs-toggle="modal" data-bs-target="#salarytypesmodal" data-stid="' . $row->id . '" data-title-en="' . $row->title_en . '" data-title-hi="' . $row->title_hi . '" data-title-gj="' . $row->title_gj . '" href="javascript:;">' . trans('labels.edit') . '</a></li></ul>';
                $bulkData['rows'][] = [
                    'id' => $cnt++,
        ...
        return view('admin.salarytypes.index');
    }
    public function store(Request $request)
    {
        if ($request->filled('salarytypesid')) {
            $ca = SalaryTypes::where('id', $request->salarytypesid)->first();
            if (empty($ca)) {
                return response()->json(['status' => 0, 'message' => trans('messages.invalid_salarytypes')], 200);
            }
        }

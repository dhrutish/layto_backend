@extends('admin.layout.default')
@section('content')
    <div class="content">
        @include('admin.layout.breadcrumb')
        <div class="card">
            <div class="card-body">
                <table class="table-responsive" id="table_newsfeeds" data-url="{{ request()->url() }}" data-toggle="table"
                    data-show-copy-rows="true" data-side-pagination="server" data-pagination="true"
                    data-page-list="[5, 10, 20, 50, 100, 200,All]" data-search="true" data-show-export="true"
                    data-mobile-responsive="true" data-sort-name="id" data-sort-order="desc"
                    data-pagination-successively-size="5" data-export-types='["xlsx"]'
                    data-export-options='{ "fileName": "<?= basename(parse_url(request()->url(), PHP_URL_PATH)) . '-' .
                    date('d-m-y') ?>", "ignoreColumn": ["action"] }' data-query-params="queryParams">
                    <thead>
                        <tr>
                            <th data-field="id" data-sortable="true" data-visible="true" data-width="80" data-width-unit="px">{{ trans('labels.srno') }}</th>
                            <th data-field="image" data-sortable="false" data-visible="true"> {{ trans('labels.image') }}</th>
                            <th data-field="industry" data-sortable="false" data-visible="true"> {{ trans('labels.industry_type') }} </th>
                            <th data-field="title" data-sortable="true" data-visible="true"> {{ trans('labels.title') }} </th>
                            <th data-field="description" data-sortable="true" data-visible="true"> {{ trans('labels.description') }} </th>
                            <th data-field="created_at" data-sortable="true" data-visible="true"> {{ trans('labels.created_at') }} </th>
                            <th data-field="top_feed" data-sortable="false" data-visible="true"> {{ trans('labels.top_feed') }} </th>
                            <th data-field="action" data-sortable="false" data-visible="true"> {{ trans('labels.action') }}
                            </th>
                        </tr>
                    </thead>
                </table>
            </div>
        </div>
    </div>
    <div class="modal fade" id="newsfeedsmodal" tabindex="-1" data-bs-backdrop="static"
        aria-labelledby="newsfeedsmodalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="newsfeedsmodalLabel" data-edit-title="{{ trans('labels.edit') }}"
                        data-add-title="{{ trans('labels.create') }}"></h5>
                    <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="needs-validation" novalidate="" method="post" data-next="{{ route('news-feeds.store') }}"
                    id="newsfeedsform">
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="newsfeedsid" id="newsfeedsid">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="industry"
                                        class="form-label required">{{ trans('labels.industry_type') }}</label>
                                    <select class="form-control" name="industry" id="industry">
                                        <option value="" selected disabled>{{ trans('labels.select') }}</option>
                                        @foreach ($itypes as $itype)
                                            <option value="{{ $itype->id }}">{{ $itype->title_en }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="title" class="form-label required">{{ trans('labels.title') }}</label>
                                    <input type="text" class="form-control" name="title" id="title"
                                        placeholder="{{ trans('labels.title') }}">
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="description"
                                        class="form-label required">{{ trans('labels.description') }}</label>
                                    <textarea class="form-control" name="description" id="description" placeholder="{{ trans('labels.description') }}"
                                        rows="8"></textarea>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="image" class="form-label required">{{ trans('labels.image') }}</label>
                                    <input type="file" class="form-control" name="image" id="image">
                                </div>
                                <img src="" alt="" id="imagefile" class="d-none" height="100">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-outline-danger" type="reset"
                            data-bs-dismiss="modal">{{ trans('labels.cancel') }}</button>
                        <button class="btn btn-primary" type="submit">{{ trans('labels.submit') }}</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
@endsection
@section('scripts')
    <script src="{{ url('resources/views/admin/newsfeeds/newsfeeds.js') }}"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/ckeditor/4.12.1/ckeditor.js"></script>
    <script type="text/javascript">
        $(function(params) {
            CKEDITOR.replace('description', {
                height: '200',
            });
            setTimeout(() => $('#cke_46').remove(), 400);
        })
    </script>
@endsection
